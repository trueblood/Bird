       ...
       # data['Timestamp'] = data['Timestamp'].dt.strftime('%Y-%m-%d %H:%M:%S')
        ...
        for species in data['Species'].unique():
            ...
            if not species_data.empty:
                ...
                overall_max_timestamp = pd.to_datetime( )
                species_max_timestamp = pd.to_datetime(species_max_timestamp)
                print(overall_max_timestamp)
                print(species_max_timestamp)
                days_to_forecast = (overall_max_timestamp - species_max_timestamp).days + 10
                ...
                forecast_dates = pd.date_range(start=species_max_timestamp + pd.Timedelta(days=1), periods=days_to_forecast, freq='D')
                future_collisions.index = forecast_dates  # Assign forecast dates
                ...
                fig.add_trace(go.Scatter(x=forecast_dates, y=future_collisions, mode='lines+markers', name=f'{species} - Forecast', line=dict(dash='dot')))
